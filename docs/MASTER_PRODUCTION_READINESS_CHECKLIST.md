...
| API-06 | Async/concurrency strategy. | Partial | P1 | Queue overload returns 429 with retry guidance; inference caller wait is bounded with timeout response. True process isolation/load testing remains external/future. |
...
| WEB-09 | Mobile Safari/Android/device coverage. | Partial | P1 | MIME negotiation supports VP9/VP8/WebM/MP4 and upload fallback with tests. Real iOS/Android device verification remains external. |
...
| TEST-10 | Performance/load/concurrency | Partial | P1 | Queue overload and timeout behavior are tested; full production load/memory testing remains external. |
...
| NOW-02 | Add countdown, max recording length, timer, auto-stop, review flow. | P1 | Browser MediaRecorder | UI has countdown/timer/auto-stop/review; component tests cover camera flow and MIME variants. Real devices remain external. | Partial |
...
| NOW-05 | Improve result/history details using trustworthy fields. | P2 | Existing API response | Authenticated results use server `analysis_session_id` when present, reconcile Supabase history, and label local/demo/unsaved rows visibly. Result panel includes timing, quality, probabilities, audio fallback. | Done |
| NOW-06 | Add safe feedback API/UI/schema/migration/docs. | P0 | None for local/mock | Feedback now requires durable persistence, verified analyses, explicit consent, and retained evidence before review candidacy. Product feedback has a dedicated `product_feedback` table and cannot enter ML feedback exports. Live Supabase Storage remains external. | Partial |
| NOW-07 | Add feedback tests: auth/forgery/consent/duplicates/labels. | P0 | TestClient | Backend and frontend tests cover no fake saved state, truthful duplicate/metadata messages, auth-required consent, evidence gating, disabled capability, forged trusted fields, missing/cross-user analysis IDs, and reset behavior. Live Supabase RLS tests remain external. | Partial |
...
| NOW-16 | Add structured logging, timing, safer proxy config, timeout/limits docs. | P1 | None | Request timing/log metadata, process-time header, trusted proxy-hop parsing, persistence timeout, queue overload response, inference timeout, and rate-limit adapter docs/tests added. | Done |
| NOW-18 | Gate model-improvement candidates on retained reviewable evidence. | P0 | Optional storage env | Consented feedback without retained/unexpired evidence is saved as non-eligible metadata; only retained evidence can enter candidate review. `SMART_CRICKET_ALLOW_MODEL_IMPROVEMENT_PARTICIPATION=false` disables retention and frontend consent controls. | Done |
