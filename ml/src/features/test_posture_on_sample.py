"""Run posture feature extraction on one real processed pose sequence (Phase 5.5 smoke test)."""
...
from feature_config import POSTURE_FEATURES
...
def main() -> None:
    ...
    if list(out.keys()) != list(POSTURE_FEATURES):
        errors.append(
            "output keys or order do not match POSTURE_FEATURES "
            f"(got {list(out.keys())})"
        )

    if set(out.keys()) != set(POSTURE_FEATURES):
        errors.append("output key set does not match POSTURE_FEATURES")

    for name, val in out.items():
        if not _is_finite_number(val):
            errors.append(f"{name} is not a finite number: {val!r}")

    if float(out["stance_width_mean"]) < 0.0:
        errors.append(
            f"stance_width_mean must be non-negative, got {out['stance_width_mean']}"
        )

    if float(out["head_stability"]) < 0.0:
        errors.append(
            f"head_stability must be non-negative, got {out['head_stability']}"
        )
    ...
    print("\nSuccess: posture features extracted from real sample.")
