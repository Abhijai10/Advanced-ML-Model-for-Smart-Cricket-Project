...
export async function analyzeVideo(
  ...
  const body = (await response.json().catch(() => null)) as
    | AnalysisResponse
    | { detail?: { detail?: string; error_code?: string } }
    | null;
  if (!response.ok) {
    const detail =
      body && 'detail' in body && typeof body.detail === 'object' ? body.detail : undefined;
...
export async function createAnalysisJob(
  ...
  filename: string,
  accessToken?: string
): Promise<Pick<AnalysisJob, 'job_id' | 'status'>> {
  const payload = new FormData();
  payload.append('file', file, filename);
...
export function openAnalysisJobWebSocket(jobId: string, accessToken?: string): WebSocket {
