...
export default function SessionsPage() {
  ...
  useEffect(() => {
    ...
    void getAnalytics<{ sessions: { id: string; accuracy: boolean | null }[] }>('session-history', session.access_token)
      .then(({ sessions }) => setAccuracyBySession(Object.fromEntries(sessions.filter((item) => item.accuracy !== null).map((item) => [item.id, item.accuracy ? 100 : 0]))))
  ...
  const sessions = rows.flatMap((row) => {
    ...
    return [{ id: row.id, date: formatDate(row.created_at), duration: durationLabel(row.shot_duration_seconds), totalShots: 1, accuracy, dominantShot: shot, shotBreakdown: [{ shot, count: 1, accuracy }], trend: 'neutral' as const, trendValue: '—', feedback: row.coaching_tips?.join(' ') || row.full_result?.detailed_feedback || 'No feedback was generated.' }];
  });
  return (
